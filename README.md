...
### `Outputs`
他のCloudFormationテンプレートや`.circle/config.yml`で情報を取得できるように、出力させる情報をこのブロック内に記述する。
...
### CloudFormationテンプレートからパラメータストアに保存した情報を他のテンプレートでパラメータストアから参照する
...
VPCIDParameter:
    ...
    Properties: 
      ...
      Value: !Ref EC2VPC
```
`Name`の値はパラメータストアに保存する情報で、上記で解説した「動的なリソース名をつける」を使って`Prefix`の部分を動的な値にしている。`Value`は参照する側のテンプレートで必要になる情報を取得している。
